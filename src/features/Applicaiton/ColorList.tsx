import React, { useEffect } from 'react'
import { addSolidColor, getSolidColorList } from '../../app/handlingDatabase'
import Color from './Color'
import * as Popover from '@radix-ui/react-popover';
import { motion } from 'framer-motion';
import { HiPlus } from 'react-icons/hi';
import { ColorPicker, useColor } from 'react-color-palette';
const ColorList: React.FC = () => {
    const [colorList, setColorList] = React.useState<IDBValidKey[]>([])
    useEffect(() => {
        const t = async () => {
            const list = await getSolidColorList()
            setColorList(list
                // .slice(0, 4)
            )
            console.log(list)
        }
        t().then(() => {
            setLoaded(true)
        })
    }, [])
    function hashCode(str:string) {
        let hash = 0;
        for (let i = 0; i < str.length; i++) {
            const char = str.charCodeAt(i);
            hash = (hash << 5) - hash + char;
            hash |= 0; // Convert to 32-bit integer
        }
        return hash;
    }
    const deleteItem = (id: string) => {
        setColorList(colorList.filter((item) => item !== id))
    }
    const [loaded, setLoaded] = React.useState(false)
    const [solidColor, setSolidColor] = useColor("hex", "#ff0000");
    return (
        <div className='w-full flex flex-row overflow-x-scroll  h-full no-scrollbar 
            '
            style={{
                scrollSnapAlign: 'start',
                scrollSnapType: 'x mandatory',
                scrollBehavior: 'smooth',
                overflow: 'hidden',
            }}
        >
            {colorList.map((colorId, index) => {
                return (
                    <Color id={String(colorId)}
                        delay={0.2 * index}
                        key={hashCode(colorId.toString())}
                        deleteItem={deleteItem}
                    />
                )
            }
            )}
            {
                loaded &&
                (
                    <Popover.Root>
                        <motion.button
                            initial={{
                                scale: 0,
                            }}
                            animate={{
                                scale: 1,
                                transition: {
                                    duration: 0.2,
                                    delay: 0.2 * colorList.length,
                                }
                            }}
                            whileHover={{
                                scale: 21 / 20,
                                x: "0.5rem",
                                y: (21 / 20) / 2 * 0.5 + "rem",
                            }}
                            transition={{
                                duration: 0.5,
                                ease: 'easeInOut',
                            }}
                            style={{
                                background: 'rgba(255,255,255,0.1)',
                                height: "14rem",
                                width: '20rem',
                                display: 'inline-block',
                                justifyContent: 'center',
                                alignItems: 'center',
                                flex: "0 0 auto",
                                margin: '0 0.5rem',
                            }}
                            className='w-full h-full'
                        >
                            <Popover.Trigger>
                                <motion.div
                                    initial={{
                                        scale: 0,
                                    }}
                                    animate={{
                                        scale: 1,
                                        rotate: 90,
                                        transition: {
                                            duration: 0.2,
                                        }
                                    }}
                                    whileHover={{
                                        scale: 21 / 20,
                                        rotate: 180,
                                        transition: {
                                            duration: 0.2,
                                        }
                                    }}
                                    className='w-full h-full flex justify-center items-center'
                                >
                                    <HiPlus className='text-white text-9xl'
                                    />
                                </motion.div>

                            </Popover.Trigger>
                        </motion.button>
                        <Popover.Content className='rounded-xl 
                        '
                            style={{
                                backgroundColor: solidColor.hex,
                                height: '50vh',
                                width: '50vw',
                            }}
                        >
                            <div
                                className=' w-full h-full flex flex-col justify-center items-center py-4
                            '

                            >
                                <div className='flex flex-row justify-center items-center w-full h-full
                                '
                                >
                                    <div>

                                        <ColorPicker width={
                                            window.innerWidth > 450 ? 450 : window.innerWidth - 50
                                        } height={
                                            window.innerWidth > 450 ? 228 : (window.innerWidth - 50) / 2
                                        }
                                            color={solidColor}
                                            onChange={setSolidColor} hideRGB hideHSV dark

                                        />
                                    </div>
                                </div>

                                <Popover.Close className="PopoverClose" aria-label="Close">

                                    <button className=' rounded-xl px-4 py-2
                            '
                                        style={{
                                            background: "#010101",
                                            color: "#fff",


                                        }}
                                        onClick={async () => {
                                            addSolidColor(solidColor.hex)
                                            let f = true
                                            while (f) {
                                                let LIST = await getSolidColorList()
                                                if (LIST.length > colorList.length) {
                                                    let diff = LIST.filter(x => !colorList.includes(x));
                                                    setColorList([...colorList, ...diff])
                                                    f = false
                                                }
                                            }
                                        }}
                                    >
                                        Add
                                    </button>
                                </Popover.Close>
                            </div>

                        </Popover.Content>
                    </Popover.Root>
                )
            }
        </div>
    )
}

export default ColorList